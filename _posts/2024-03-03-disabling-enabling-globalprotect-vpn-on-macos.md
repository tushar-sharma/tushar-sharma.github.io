...
If that does not work for the GUI LaunchAgent session, try the Aqua session option:

```sh
launchctl unload -S Aqua /Library/LaunchAgents/com.paloaltonetworks.gp.pangp*
```
...
Or, if you used the Aqua session option:

```sh
launchctl load -S Aqua /Library/LaunchAgents/com.paloaltonetworks.gp.pangp*
```
