...
proc reg data=YourData;
  ...
  model DependentVariable = IndependentVariable1 IndependentVariable2;

  output out=ModelOutput predicted=PredictedValues rstudent=StudentizedResiduals;
...
```sas
proc print data=ModelOutput noobs;
  var StudentizedResiduals;
run;
...
```sas
# Assuming you have a fitted regression model object called 'model'

# Replace 'model' with the actual name of your regression model object


# Step 1: Calculate the studentized residuals

studentized_res <- rstandard(model)
```
...
```
# Step 2: Print the studentized residuals
