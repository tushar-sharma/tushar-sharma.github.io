...
Model quality depends heavily on training data quality. "Quality" does not just mean clean grammar or correct labels. It also means that the data covers the tasks, languages, domains, edge cases, and user groups the model is expected to handle. A model trained on narrow or biased data will often learn narrow or biased behavior, no matter how large the architecture is.

CLIP is a multimodal model that learns from pairs of images and text captions. Instead of only learning to classify images into fixed labels, it learns to place related images and text near each other in the same embedding space. For example, an image of a dog and the caption "a dog playing in grass" should end up close together, while an unrelated caption should be farther away.

A model with 7B parameters has roughly 7 billion learned weights. A weight is a number adjusted during training. Parameter count is not the same as intelligence, but it affects model capacity, memory usage, and serving cost.

**Fine-tuning types**:

1. **Instruction fine-tuning**: The input is usually a pair of instruction and response. The goal is to teach a pretrained model to follow user instructions in a helpful format.

2. **Preference fine-tuning**: The input is usually an instruction, a preferred response, and a rejected response. The goal is to teach the model which outputs humans or reward models prefer.

**Ossification**: Pretraining can make a model's behavior hard to change later. The model may have learned representations or habits that fine-tuning only weakly updates. Possible mitigations include better fine-tuning data, larger or more targeted updates, adapters such as LoRA, continued pretraining on domain data, or training recipes that avoid overfitting too early.

Small perturbations can make a model misclassify an input. In vision models, even changing a few pixels, or adding carefully chosen noise that humans barely notice, can change the predicted class. This is related to adversarial examples. A "single-pixel attack" is one specific kind of perturbation, not the general name for all noisy inputs.

**Position bias** means the model or ranking system favors an option partly because of where it appears, not because it is truly better. For example, an evaluator might prefer the first answer in a pairwise comparison more often than it should. To reduce position bias, randomize answer order, evaluate each pair in both orders, hide irrelevant metadata, and measure whether preferences flip when positions are swapped.

Code generated by Codex, Claude, or another AI agent should go through deterministic guardrails before it is trusted: parsers, formatters, linters, type checkers, tests, security scans, and human review. The model output is probabilistic, but these checks are rule-based and make failures easier to catch.

Synthetic data can be used to train or fine-tune a model, especially when real data is scarce, sensitive, or expensive to label. The risk is that synthetic data can amplify the generating model's errors or blind spots, so it needs filtering, evaluation, and ideally some grounding in real examples.
